...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'y':
		...
		break;
	case 32: // space key
		if (velocity < 0.00000001 && velocity > -0.00000001)
		{
	...
	void testStairCollision(bool axe, double xmin, double xmax, double zmin, double zmax, double ymin, double ymax)
	{
		double axei = axe ? Ez : Ex;
		double min = axe ? zmin : zmax;
		double max = axe ? zmax : xmax;
		double y = ((ymax - ymin) / (max - min) * axei) - 1 + ((ymax - ymin) / (max - min));
		// if(E<y && E > y -3) {
		// 	E = y;
		// 	velocity = 0;
		// }
		...
		glColor3f(1, 0, 0);
		glBegin(GL_QUADS);
		glVertex3f(xmin, 1, zmin);
		glVertex3f(xmin, 1, zmax);
		glVertex3f(xmax, 1, zmax);
		glVertex3f(xmax, 1, zmin);
		glEnd();
		glPopMatrix();
	}
	...
	void testCollision()
	{
		...
		// testStairCollision(Z_AXE, 331, 343, -174.8, -144, 0, 38.5/2.0);
